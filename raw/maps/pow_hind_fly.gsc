...
start()
{
	...
	level thread maps\pow_compound::start(); //-- Start the next section of the map
}
...
village_1_spawn_trucks()
{
	...
	maps\_vehicle::scripted_spawn(26);
	wait(0.05);
	maps\_vehicle::scripted_spawn(27); //-- boats!
	wait(0.05);
	//-- spawn in the trucks
	for(i = 19; i < 23; i++)
	{
		trucks = maps\_vehicle::scripted_spawn( i );
		wait(0.05);
...
truck_depot_drones( building_script_noteworthy, drone_trig_script_noteworthy, start_spawn_notify, wait_delay )
{
	...
	AssertEX( IsDefined(building), "Building for drone spawn does not exist");
	
	drone_trig = GetEnt(drone_trig_script_noteworthy, "script_noteworthy");
	AssertEX( IsDefined(drone_trig), "Drone trigger does not exist" );
...
watch_for_bend_village()
{
	...
	flag_wait("village_1_vehicles_spawned");
	maps\_vehicle::scripted_spawn(29);
	wait(0.05);
	...
	trigger_wait( "trig_kick_off_village_1_anyway" );
	maps\_vehicle::scripted_spawn( 23 ); //-- the trucks that are just spawned in and sit
...
watch_for_2nd_village()
{
	...
	add_spawn_function_veh( "big_village_quad", maps\pow_utility::zsu_basic_fire, level.hind, "alert_all_drones", 1);
	...
	maps\_vehicle::scripted_spawn(25); //-- the 4 ZSUs
	maps\_vehicle::scripted_spawn(26); //-- the boat at the dock
...
spawn_more_convoy_behind_me( other_vehicles, delay, spawn_func, param1 ) //-- The primary vehicle
{
	...
	{	
		...
		AssertEX( IsDefined(new_vehicle), "Had trouble spawning the new vehicle for the convoy in spawn_more_convoy_behind_me" );
...
{
	AssertEx( IsDefined(self.script_noteworthy), "The troop truck did not have a valid script noteworthy" );
...
hind_vs_hip()
{
	...
	level.enemy_hip = GetEnt("enemy_hip", "targetname");
	AssertEX(IsDefined(level.enemy_hip), "The enemy hip doesn't exist");
...
hind_vs_hinds()
{
	...
	maps\_vehicle::scripted_spawn( 18 );
...
{
	add_spawn_function_veh( "nva_med_patrol", ::nva_med_boat_shoot_in_range, "alert_all_drones" );
}
