...
start_tunnel_chase()
{
	...
	level.woods enable_cqbsprint(); //-- uses tighter turns
	...
	spetz_escape_scene( level.russian ); //TODO: come back and look at this to see if we have a living spetz that we can use
	...
	level thread maps\pow_hind_clearing::start_non_demo();
...
tunnel_machete_attack_animate()
{
	...
	struct thread anim_first_frame( self, "machete_over");
...
spetz_escape_scene( spetz )
{
	...
	level.russian SetCanDamage(false);
	
	level thread kill_all_other_ai( level.russian ); //-- pass the exception
	...
	player_body Hide();
	player thread take_and_giveback_weapons("climb_up_scene_done");
	...
	player Unlink();
	player_body Delete();
	player notify("climb_up_scene_done");
...
woods_kill_russian( delay )
{
	...
	level.woods thread shoot_at_target_untill_dead(level.russian);
...
woods_VO_tunnel_fail()
{
	...
	level.woods anim_single( level.woods, "wakeup" );
	level.after_russian_VO = false;
	//level.scr_sound["barnes"]["wakeup"] = "vox_cub1_s02_062A_wood";
}
